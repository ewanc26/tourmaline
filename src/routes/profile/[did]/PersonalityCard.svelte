...
<div class="rounded border border-[var(--border)] bg-[var(--surface)] p-4 sm:p-6">
	...
	<div class="flex items-start justify-between gap-3">
		<div class="min-w-0">
			<p class="text-xs font-medium tracking-wide text-[var(--text-dim)] uppercase">Listener archetype</p>
			<h2 class="mt-1 text-xl font-bold text-[var(--accent)] sm:text-2xl">{personality.archetype}</h2>
			<p class="mt-1 text-sm leading-relaxed text-[var(--text-muted)]">{personality.archetypeBlurb}</p>
		</div>
		<div class="flex shrink-0 flex-col items-end gap-1">
			<span class="rounded border border-[var(--border)] bg-[var(--bg)] px-2 py-0.5 font-mono text-xs text-[var(--text-muted)]">
				Diversity {profile.diversityScore}
			</span>
			<span class="rounded border border-[var(--border)] bg-[var(--bg)] px-2 py-0.5 font-mono text-xs text-[var(--text-muted)]">
				Obscurity {profile.obscurityIndex}
			</span>
		</div>
	</div>
	...
	{#if topGenres.length > 0}
		<div class="mt-5">
			<p class="mb-2 font-mono text-xs uppercase tracking-wide text-[var(--text-dim)]">Genre profile</p>
			<div class="space-y-1.5">
				{#each topGenres as genre (genre.name)}
					<div class="flex items-center gap-2">
						...
						<div class="flex-1 h-2 rounded-full bg-[var(--bg)]">
							<div
	...
	{#if topMoods.length > 0}
		<div class="mt-4">
			<p class="mb-2 font-mono text-xs uppercase tracking-wide text-[var(--text-dim)]">Mood profile</p>
	...
	<ul class="grid gap-2.5 sm:grid-cols-2 lg:grid-cols-3">
		{#each personality.traits as trait (trait.label)}
			<li class="rounded border border-[var(--border)] bg-[var(--bg)] px-3 py-2.5">
				<p class="font-mono text-[10px] uppercase tracking-wide text-[var(--accent-dim)]/60">{trait.label}</p>
				<p class="mt-0.5 text-sm font-semibold text-[var(--text)]">{trait.value}</p>
